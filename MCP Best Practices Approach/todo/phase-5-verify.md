...
- [x] **[naming regression guard]** `make scrub-check` (`scripts/scrub-check.sh`) — case-insensitive grep for the Customer's company name across all git-tracked files; wired into `tests/runUnitTests.sh` so it runs on every unit-test pass. Green as of this work.
- [x] **[test counts reconciled]** README.md and CLAUDE.md now cite the current totals: **245 test functions across 15 packages** (mesh 80, mcp-proxy 81, agent 37, splunk-mock 8, datadog-mock 11, apim-mcp 11, mi-mcp 9, is-mcp 8). Verify anytime with `./tests/runUnitTests.sh` (authoritative per-package footer).
- [ ] **[open hardening items from technical-requirements analysis]** Before declaring the demo production-grade, the following implementation gaps must be resolved. **(1) propose-before-act code-level gate — ✅ DONE** (Phase 4 §4.9, `code/agent/approval.bal`: `run_runbook` intercepted; only `approve <token>` executes; live-verified). Still open: (2) typed `DiagnosisResult` output contract, (3) trace-ID width reconciliation in `correlate_trace`, (4) SSE vs synchronous steps decision for `run_runbook`, (5) audit-log and deploy-freeze durability. Each remaining item is acceptable as-is for the POC demo but is a known gap for any follow-on production work.
